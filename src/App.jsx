...
export default function App(){
  const [form, setForm] = useState({
    ticker: 'AAPL', shares: 100, entry_price: 160,
    put_strike: 150, call_strike: 175, expiration: ''
  })
  ...
  const onChange = (e)=>{
    ...
    setForm(f=>({ ...f, [name]: name==='shares'? Number(value||0): ['entry_price','put_strike','call_strike'].includes(name)? Number(value||0): value }))
  }
  ...
  const submit = async ()=>{
    ...
    try{
      ...
      chartObj.current = new Chart(ctx, {
        type:'line',
        data:{ labels: data.payoff_prices, datasets:[{ label:'Payoff at Expiration', data: data.payoff_values, tension:.15 }] },
        options:{ responsive:true, plugins:{ title:{ display:true, text:`${data.ticker} Collar Payoff` }}, scales:{ x:{ title:{ display:true, text:'Stock Price at Expiration' }}, y:{ title:{ display:true, text:'P/L ($)'} } } }
      })
    ...
    <div className="wrapper">
      ...
      <div className="card">
        <div className="grid">
          ...
          <div><div className="label">Expiration</div><select className="input" name="expiration" value={form.expiration} onChange={onChange}>{exps.map(e=> <option key={e} value={e}>{e}</option>)}</select></div>
        </div>
        ...
        {res && (
          <>
            <div className="kpis">
              {kpis.map(k=> (<div key={k.title} className="kpi"><div className="title">{k.title}</div><div className="val">{k.val}</div></div>))}
            </div>
            ...
            <div className="chart"><canvas ref={chartRef} height="220" /><div className="note">Payoff uses delayed data and mid-pricing for premiums.</div></div>
          </>
        )}
      </div>
    </div>
